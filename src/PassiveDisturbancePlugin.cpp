...
public:
  PassiveDisturbancePlugin() = default;
...
public:
  void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override {
    ...
    const std::string body_name = sdf->GetElement("bodyName")->Get<std::string>();
    this->link_ = this->model_->GetLink(body_name);
    if (!this->link_) {
      gzerr << "passive disturbance plugin could not find link " << body_name << "\n";
      return;
    }
    ...
    this->update_connection_ =
        event::Events::ConnectWorldUpdateBegin([this](const common::UpdateInfo& info) { this->Update(info); });
  }

private:
  ignition::math::Vector3d ReadVector3(const sdf::ElementPtr& sdf, const std::string& name) const {
...
private:
  void Update(const common::UpdateInfo& info) {
    ...
    this->link_->AddRelativeForce(this->force_mean_ + (this->force_amplitude_ * wave));
    this->link_->AddRelativeTorque(this->torque_mean_ + (this->torque_amplitude_ * wave));
  }
...
private:
  ignition::math::Vector3d force_mean_;

private:
  ignition::math::Vector3d force_amplitude_;

private:
  ignition::math::Vector3d torque_mean_;

private:
  ignition::math::Vector3d torque_amplitude_;
