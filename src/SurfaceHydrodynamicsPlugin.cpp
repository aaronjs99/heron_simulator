...
class SurfaceHydrodynamicsPlugin : public ModelPlugin
{
  public: void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override
  {
    ...
    this->linear_damping_ = this->ReadVector3(
        sdf, "linearDamping", ignition::math::Vector3d(25.0, 24.0, 150.0));
    this->quadratic_damping_ = this->ReadVector3(
        sdf, "quadraticDamping", ignition::math::Vector3d(5.0, 5.0, 15.0));
    this->angular_damping_ = this->ReadVector3(
        sdf, "angularDamping", ignition::math::Vector3d(20.0, 20.0, 10.0));
    this->angular_quadratic_damping_ = this->ReadVector3(
        sdf, "angularQuadraticDamping", ignition::math::Vector3d(8.0, 8.0, 8.0));
    ...
    this->update_connection_ = event::Events::ConnectWorldUpdateBegin(
        [this](const common::UpdateInfo&) { this->Update(); });
  }

  private: static double Drag(double velocity, double linear, double quadratic)
  {
  ...
  private: void Update()
  {
    ...
    const double hydrostatic_n = std::max(
        ...
        std::min(
            2.0 * weight_n,
            weight_n + this->heave_stiffness_ * (this->equilibrium_z_ - pose.Pos().Z())
    ...
    this->link_->AddRelativeForce(ignition::math::Vector3d(
        this->Drag(velocity_body.X(), this->linear_damping_.X(),
                   this->quadratic_damping_.X()),
        this->Drag(velocity_body.Y(), this->linear_damping_.Y(),
    ...
    this->link_->AddRelativeTorque(ignition::math::Vector3d(
        -this->roll_stiffness_ * euler.X()
            + this->Drag(angular_body.X(), this->angular_damping_.X(),
                         this->angular_quadratic_damping_.X()),
        -this->pitch_stiffness_ * euler.Y()
            + this->Drag(angular_body.Y(), this->angular_damping_.Y(),
                         this->angular_quadratic_damping_.Y()),
        this->Drag(angular_body.Z(), this->angular_damping_.Z(),
  ...
  private: double ReadDouble(
      const sdf::ElementPtr& sdf, const std::string& name, double fallback) const
  {
      ...
      const ignition::math::Vector3d& fallback) const
  {
    return sdf->HasElement(name)
               ? sdf->Get<ignition::math::Vector3d>(name)
               : fallback;
  }
  ...
  private: double mass_kg_ = 0.0;
  private: double equilibrium_z_ = 0.0;
  private: double heave_stiffness_ = 900.0;
  private: double heave_damping_ = 180.0;
  private: double roll_stiffness_ = 35.0;
  private: double pitch_stiffness_ = 35.0;
  private: ignition::math::Vector3d linear_damping_;
  private: ignition::math::Vector3d quadratic_damping_;
  private: ignition::math::Vector3d angular_damping_;
  private: ignition::math::Vector3d angular_quadratic_damping_;
