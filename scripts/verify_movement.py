    ...
    def callback(self, msg):
        try:
            ...
            if self.start_pose is None:
                ...
                dist = ((pose.position.x - self.start_pose.position.x)**2 + (pose.position.y - self.start_pose.position.y)**2)**0.5
                print(f"T+{dt:.1f}s: x={pose.position.x:.2f}, y={pose.position.y:.2f}, dist={dist:.3f}m")
