...
class MapGTPublisher:
    ...
    def msg_to_matrix(self, msg):
        ...
        trans = [p.x, p.y, p.z]
        rot = [q.x, q.y, q.z, q.w]
        return tr.concatenate_matrices(tr.translation_matrix(trans), tr.quaternion_matrix(rot))
    ...
    def gt_cb(self, gt_msg):
        ...
        try:
            ...
            tf_stamped = self.tf_buffer.lookup_transform(self.odom_frame, self.base_frame, rospy.Time(0))
            
            p = tf_stamped.transform.translation
            q = tf_stamped.transform.rotation
            trans = [p.x, p.y, p.z]
            rot = [q.x, q.y, q.z, q.w]
            t_odom_base = tr.concatenate_matrices(tr.translation_matrix(trans), tr.quaternion_matrix(rot))
